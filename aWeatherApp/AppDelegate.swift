...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    let cities = [
        City(name: "London", countryISOCode: "UK"),
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        let list = Component.Factory.cityList(
            ...
            signalForTampratureAndIcon: { city in
                let endPointShared = WeatherEndpoint(city: city.name, isoCountryCode: city.countryISOCode)
                    .signal().shareReplay()
                
                endPointShared.toWeatherInfoModel().observeNext { weatherInfo  in
                    city.weatherInfo = weatherInfo
                }.dispose(in: self.bag)
                
                return endPointShared.endpointToTempratureAndIcon()
        })

        mainFlow = MainFlow(rootComponent: (viewController: list.viewController, didSelectCity: list.viewModel.didSelectCity))
        flow = FlowController(
            viewModel: mainFlow)
        
        window?.rootViewController = flow.root
